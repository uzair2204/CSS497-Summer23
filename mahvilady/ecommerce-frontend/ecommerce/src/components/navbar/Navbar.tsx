...
import { adminNavigations, commonNavigations, userNavigations } from "@data/navbarNavigations";
...
const Navbar: FC<NavbarProps> = ({ }) => {
  ...
  if (state?.user?.role === UserRole.USER) {
    navs = [...commonNavigations, ...userNavigations];
  } else if (state?.user?.role === UserRole.ADMIN) {
    navs = [...commonNavigations, ...adminNavigations];
  } else {
    navs = commonNavigations;
  }
  ...
  return (
    <StyledNavbar>
      <Container height="100%" display="flex" alignItems="center" justifyContent="space-between">
        ...
        <FlexBox >{renderNestedNav(navs, true)}</FlexBox>
      </Container>
    </StyledNavbar>
  );
};
