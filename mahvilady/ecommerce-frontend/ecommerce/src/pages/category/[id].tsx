...
const EditCategory = () => {
    ...
    const getNav = async () => {
        if (id) {
            setLoading(true);
            const data = await categoryApi.getNavigationById(id);
            setCat(data);
            setLoading(false);
    ...
    const initialValues = {
        ...
        children: cat?.children.map((c)=>c.id),
        ...
        if (submissionInprogress) {
            ...
            try {
                if(imageChange){
                    ...
                    await categoryApi.putWithImage(id,formData);
                }else{
                    formData.append('object', JSON.stringify(values));
                    categoryApi.put(id,formData);
                }
                                    ...
                                    <FlexBox flexDirection="row" mt={2} flexWrap="wrap">
                                        {imageChange && <UploadImageBox key={1} mr=".5rem">
                                            ...
                                            <UploadImageBox key={1} mr=".5rem">
                                                <Image src={`/assets/images/${cat.icon}`} width="100%" />
                                            </UploadImageBox>
                                        }
                                    </FlexBox>
                                    </FlexBox>
