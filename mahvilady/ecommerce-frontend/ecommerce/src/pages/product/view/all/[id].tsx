...
const ProductSearchResult = () => {
  ...
  const [selectedSortOption, setSelectedSortOption] = useState(sortOptions[3]);
  ...
  // Check if id is an array (which can happen with catch-all routes)
  const sortId = Array.isArray(id) ? id[0] : id;

  // Now you can safely parse productId as an integer
  useEffect(() => {
    const sortOrder = parseInt(sortId, 10);
  
    if (!isNaN(sortOrder) && sortOrder >= 0 && sortOrder <= 7) {
      setSelectedSortOption(sortOptions[sortOrder]);
    }
  }, [sortId]);
  ...
  const getProduct = async () => {
    if (sortId !== null) {
      ...
      let response;
      if (!applyCriteria) {
        response = await productApi.getByPaging(page, selectedSortOption.value, selectedPaginationOption.value);
      } else {
  ...
  useEffect(() => {
    getProduct();
  }, [page, selectedSortOption, selectedPaginationOption, criteria]);
...
const sortOptions = [
  ...
  { label: "Date new to old", value: "creationTimeStamp,desc" },
  { label: "Date old to new", value: "creationTimeStamp,asc" },
];
